...
static void pmw3610_async_init(struct k_work *work) {
    ...
    if (data->err) {
        ...
        if (data->retries < CONFIG_PMW3610_ALT_INIT_RETRY_COUNT) {
            data->retries++;
            data->async_init_step = ASYNC_INIT_STEP_POWER_UP;
            LOG_WRN("Retrying PMW3610 initialization (%d/%d) in %d ms",
                    data->retries, CONFIG_PMW3610_ALT_INIT_RETRY_COUNT,
                    CONFIG_PMW3610_ALT_INIT_RETRY_DELAY_MS);
            k_work_schedule(&data->init_work,
                            K_MSEC(CONFIG_PMW3610_ALT_INIT_RETRY_DELAY_MS));
        } else {
            LOG_ERR("PMW3610 initialization stopped after %d failed attempts", data->retries);
        }
    } else {
